# TODO: Translation updated at 2019-02-07 23:24

translate french strings:

    old "Students"
    new "Etudiants"
    old "Auntie"
    new "Tatie"
    old "Dad"
    new "Papa"
    old "Headmaster"
    new "Directeur"

    # script.rpy:178
    old "Spy Camera"
    new "Caméra espion"

    # script.rpy:179
    old "Camera"
    new "Appareil photo"

    # script.rpy:180
    old "Cheap Controller"
    new "Contrôleur bon marché"

    # script.rpy:181
    old "Teacher’s Break Room Key"
    new "Clé de la salle de pause des enseignants"

    # script.rpy:182
    old "Celia’s Note"
    new "Note de Celia"

    # script.rpy:183
    old "Empty Envelope"
    new "Enveloppe vide"

    # script.rpy:184
    old "Envelope"
    new "Enveloppe"

    # script.rpy:185
    old "WebCam CD"
    new "CD WebCam"

    # script.rpy:186
    old "Dildo"
    new "Gode"

    # script.rpy:187
    old "Sexy Cloth"
    new "Tissu sexy"

    # script.rpy:188
    old "Celia's Vibrator"
    new "Le vibromasseur de Celia"

    # script.rpy:189
    old "Binoculars"
    new "Jumelles"

    # script.rpy:190
    old "Caroline’s Spare Key"
    new "Clé de rechange de Caroline"

    # script.rpy:191
    old "Swimming Poll Card"
    new "Carte de Piscine"

    # script.rpy:192
    old "Red Wine"
    new "Vin rouge"

    # script.rpy:193
    old "White Wine"
    new "Vin blanc"

    # script.rpy:194
    old "Car Keys"
    new "Clés de voiture"

    # script.rpy:195
    old "Book"
    new "Livre"

    # script.rpy:196
    old "Safe Note"
    new "Note de sécurité"

    # script.rpy:197
    old "Note"
    new "Note"

    # script.rpy:198
    old "Zuri's Magnet Card"
    new "La carte magnétique de Zuri"

    # script.rpy:199
    old "Sara's Vibrator"
    new "Le vibromasseur de Sara"

    # script.rpy:200
    old "Lube"
    new "Lubrifiant"

    # script.rpy:201
    old "Liza’s Key"
    new "La clé de Liza"

    # script.rpy:202
    old "Cleaning Stuff"
    new "Produits de nettoyage"

    # script.rpy:203
    old "Condom"
    new "Préservatif"

    # script.rpy:204
    old "School Permission"
    new "Dérogation de l'école"

    # script.rpy:205
    old "Drinks"
    new "Boissons"

    # script.rpy:206
    old "Ice Creams"
    new "Crème glacée"

    # script.rpy:207
    old "SunScreen"
    new "Écran solaire"

    # script.rpy:209
    old "I’m still thinking about our… time spent together on this couch…"
    new "Je pense encore à notre… temps passé ensemble sur ce canapé…"

    # script.rpy:211
    old "Our today conversation... Made me very happy.. I hope you feel the same."
    new "Notre conversation d'aujourd'hui... Ça m'a rendue très heureuse.. J'espère que tu ressens la même chose."

    # script.rpy:213
    old "I masturbated.. I just had to do it on this couch... I just couldn't hold myself and wait for the night."
    new "Je me suis masturbée.. Je devais juste le faire sur ce canapé... Je ne pouvais pas me retenir et attendre la nuit."

    # script.rpy:216
    old "Our date tonight was amazing! It’s a shame we can't continue it in my bed.. :( Love you"
    new "Notre rendez-vous de ce soir était génial ! C'est dommage qu'on ne puisse pas continuer dans mon lit.. :( Je t'aime"

    # script.rpy:218
    old "I’ll not let you steal my panties from me… I like them."
    new "Je ne te laisserai pas me voler ma culotte... Je les aime bien."

    # script.rpy:220
    old "I hope you're not still sad that you lost all those matches…? Lily told me to send you this.."
    new "J'espère que tu n'es pas encore triste d'avoir perdu tous ces matchs…? Lily m'a dit de t'envoyer ça.."

    # script.rpy:222
    old "Today was the best day of my life <3"
    new "Aujourd'hui était le plus beau jour de ma vie <3"

    # script.rpy:225
    old "I think I'm gonna start to learn how to swim! Cheer me on <3"
    new "Je crois que je vais commencer à apprendre à nager ! Encouragez-moi <3"

    # script.rpy:227
    old "Look... I'm practicing as I promised... "
    new "Regarde... Je m'entraîne comme promis... "

    # script.rpy:229
    old "I'm so sorry... She surprised me! Please, don't be angry at me... :("
    new "Je suis désolée... Elle m'a surpris ! S'il te plaît, ne sois pas en colère contre moi... :("

    # script.rpy:231
    old "My ass still hurts... kinda... I hope you loved it <3"
    new "J'ai encore mal au cul... un peu.... J'espère que tu as aimé <3"

    # script.rpy:233
    old "Zuri here. Come to our place in the evening..."
    new "Zuri à l'appareil. Viens chez nous le soir..."

    # script.rpy:235
    old "Zuri here! Just look at Suri's ass! Completely defenseless… Come to our place in the evening..."
    new "Zuri à l'appareil ! Regarde le cul de Suri ! Complètement sans défense.... Venez chez nous le soir...."

    # script.rpy:239
    old "YOU GOT CUM ON THE PANTIES THAT ARE SUPPOSED TO BE SOLD! YOU IDIOT! Just kidding <3"
    new "TU AS DU SPERME SUR LA CULOTTE QUI EST CENSÉE ÊTRE VENDUE ! IDIOT ! Je plaisante <3"

    # script.rpy:241
    old "Wh-y would ansyone want to roobed my shosp?  ;( Tht life is not fair.."
    new "Qu'est-ce qu'on voudrait me dévorer ? ;( La vie n'est pas juste.."

    # script.rpy:243
    old "Thanks for helping me with money. It means a lot to me. Kisses :* :* Just don't jerk off to my pic haha :P"
    new "Merci de m'aider avec l'argent. C'est très important pour moi. Kisses :* :* Ne te branle pas sur ma tof haha :P"

    # script.rpy:244
    old "You're stupid..."
    new "Tu es stupide..."

    # script.rpy:246
    old "You've asked about a date while we were at that nightclub... Let's go tomorrow!"
    new "Tu m'as demandé un rencard quand on était dans cette boîte de nuit... Allons-y demain !"

    # script.rpy:647
    old "Take a nap."
    new "Faire une sieste."

    # script.rpy:647
    old "Sleep until next day."
    new "Dormir jusqu'au lendemain."

    # script.rpy:647
    old "Weekend Event."
    new "Event du Weekend."

    # script.rpy:647
    old "Skip weekend."
    new "Sauter le Weekend."

    # script.rpy:647
    old "Back."
    new "Retour."